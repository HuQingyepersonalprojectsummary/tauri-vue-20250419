...
- 🌐 **IPv6 双栈网络完整读取与配置**：
  - 匹配 Windows 11 “编辑 IP 设置”规范，提供独立的 Fluent 风格拨动开关控制 IPv6 (`ms_tcpip6`) 协议组件；
  - **IPv6 地址读取与配置**：支持 DHCP / 路由器通告 (SLAAC) 自动获取与静态手动配置（静态 IPv6 地址、前缀长度及默认网关）；
  - **IPv6 DNS 读取与配置**：支持 DHCPv6 自动获取与静态指定，内置阿里、腾讯、百度、Cloudflare、Google 等常用 IPv6 DNS 一键预设；
  - IPv6 完整纳入现场快照保护与事务性回滚闭环。
- ⚡ **常用公共 DNS / DoH / IPv6 快捷预设**：
  - 内置阿里公共 DNS、腾讯 DNSPod、Cloudflare、Google 等主流 IPv4/IPv6 服务，支持一键填入地址与加密模板。
- 🛡️ **读回校验与自动事务回滚**：
  - 变更前采集完整快照（覆盖 IPv4/IPv6 地址、掩码/前缀、网关、DNS、IPv6 绑定与 DoH 配置）；
  - 应用后循环读回系统真实状态进行深度比对；
