...
}
{
  ...
  state.selectedAdapter.value = 'B'; await flush();
  state.selectedAdapter.value = 'A'; await flush();
  results.push({ id: 'R-04-residual', case: 'cached adapter snapshot identity', selected: state.selectedAdapter.value, snapshotAdapter: state.currentSnapshot.value?.adapterName });
  state.fillFromHistory({ adapter: 'deleted-adapter', ip: '203.0.113.30', mask: '255.255.255.0' }); await flush();
  await state.applyConfig();
  results.push({ id: 'R-04-residual', case: 'missing history adapter silently targets current selection', selected: state.selectedAdapter.value, form: {...state.ipConfig} });
  scope.stop();
}
