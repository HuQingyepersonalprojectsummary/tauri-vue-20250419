...
fn main() {
  ...
  match std::env::args().nth(1).as_deref() {
    Some("sleep") => {
      ...
      std::fs::write(exe.with_extension("descendant-finished"), "finished after timeout").unwrap();
      return;
    },
    Some("parent") => {
  ...
  println!("{}", serde_json::json!({"id":"R-05","timeoutMs":500,"elapsedMs":elapsed,"success":ok,"error":error,"descendantFinishedAfterReturn":exe.with_extension("descendant-finished").exists()}));
}
`);
