...
pub fn audit_main() {
    ...
    reset_mock("direct_rollback_dns_failure");
    let mut empty = fixture_snapshot(false);
    empty.addresses.clear();
    empty.dns_servers.clear();
    let result = serde_json::to_value(rollback_snapshot("fixture", &empty)).unwrap();
    results.push(capture("R-03", "empty_static_state_DNS_failure", result));

    reset_mock("dns_order_mode_identity");
    let cfg = Ipv4Config { adapter:"fixture".into(), ip:"192.0.2.20".into(), mask:"255.255.255.0".into(),
        gateway:"192.0.2.254".into(), dns1:"198.51.100.53".into(), dns2:"203.0.113.53".into() };
    let result = serde_json::to_value(apply_adapter_ipv4_config_transactional(&cfg)).unwrap();
    results.push(capture("R-02", "wrong_DNS_order_mode_and_adapter_identity", result));
